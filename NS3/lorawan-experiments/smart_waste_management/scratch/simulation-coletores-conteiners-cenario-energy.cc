...
Time appPeriod = Hours(1); // 30 em 30 min
...
string battery_energy_result_file = "";
string conteiner_energy_result_file = "";
...
void GetEnergyRemaining(EnergySourceContainer sources, string energy_file, double interval){
  // open log file for output
  ofstream os;
  string logFile = output_results_path + energy_file;
  ...
  Simulator::Schedule(Hours(interval), &GetEnergyRemaining, sources, energy_file, interval);
}
...
LoraPacketTracker& runSimulation(){
  ...
  BasicEnergySourceHelper basicSourceHelper_battery;
  BasicEnergySourceHelper basicSourceHelper_conteiner;

  LoraRadioEnergyModelHelper radioEnergyHelper_battery;  
  LoraRadioEnergyModelHelper radioEnergyHelper_conteiner;

  /* Battery trash bin - configure energy source */
  // values based on SX1272/73 datasheet - Table 6 Power Consumption Specification
  // Radio Tx/Rx Current + heltech and sensors
  basicSourceHelper_battery.Set ("BasicEnergySourceInitialEnergyJ", DoubleValue (10000)); // Energy in J
  basicSourceHelper_battery.Set ("BasicEnergySupplyVoltageV", DoubleValue (3.3)); // Volts
  radioEnergyHelper_battery.Set ("StandbyCurrentA", DoubleValue (0.0014)); // Ampere
  radioEnergyHelper_battery.Set ("TxCurrentA", DoubleValue (0.028)); // Ampere
  radioEnergyHelper_battery.Set ("SleepCurrentA", DoubleValue (0.0000015)); // Ampere
  radioEnergyHelper_battery.Set ("RxCurrentA", DoubleValue (0.0112)); // Ampere
  radioEnergyHelper_battery.SetTxCurrentModel ("ns3::ConstantLoraTxCurrentModel","TxCurrent", DoubleValue (0.028)); // Ampere
  
  /* Battery trash bin - configure energy source */
  // values based on SX1272/73 datasheet - Table 6 Power Consumption Specification
  // Radio Tx/Rx Current + heltech and sensors
  basicSourceHelper_conteiner.Set ("BasicEnergySourceInitialEnergyJ", DoubleValue (10000)); // Energy in J
  basicSourceHelper_conteiner.Set ("BasicEnergySupplyVoltageV", DoubleValue (3.3)); // Volts
  radioEnergyHelper_conteiner.Set ("StandbyCurrentA", DoubleValue (0.0014)); // Ampere
  radioEnergyHelper_conteiner.Set ("TxCurrentA", DoubleValue (0.028)); // Ampere
  radioEnergyHelper_conteiner.Set ("SleepCurrentA", DoubleValue (0.0000015)); // Ampere
  radioEnergyHelper_conteiner.Set ("RxCurrentA", DoubleValue (0.0112)); // Ampere
  radioEnergyHelper_conteiner.SetTxCurrentModel ("ns3::ConstantLoraTxCurrentModel","TxCurrent", DoubleValue (0.028)); // Ampere
  
  
  //Separation of nodes into batteries and containers
  NodeContainer endDevices_battery;
  NodeContainer endDevices_conteiners;
  
  for(size_t i = 0; i < unicamp_battery_bins_dataset.size(); i++){
    endDevices_battery.Add(endDevices.Get(i));
  }
  for(size_t i = unicamp_conteiner_bins_dataset.size(); i < endDevices.GetN(); i++){
    endDevices_conteiners.Add(endDevices.Get(i));
  }
 
  // install source on EDs' nodes
  EnergySourceContainer sources_battery = basicSourceHelper_battery.Install (endDevices_battery);
  EnergySourceContainer sources_conteiner = basicSourceHelper_conteiner.Install (endDevices_conteiners);
 
  // Net devices
  LoraHelper helper_battery = LoraHelper ();   // Create the LoraHelper
  LoraHelper helper_conteiner = LoraHelper ();   // Create the LoraHelper
  NetDeviceContainer netDevices_battery = helper_battery.Install (phyHelper, macHelper, endDevices_battery);
  NetDeviceContainer netDevices_conteiner = helper_conteiner.Install (phyHelper, macHelper, endDevices_conteiners);
  cout << "++++++++++++\n";
  // install device model
  DeviceEnergyModelContainer deviceModels_battery = radioEnergyHelper_battery.Install(netDevices_battery, sources_battery);
  cout << "Bug Aqui? ++++++++++++\n";
  // DeviceEnergyModelContainer deviceModels_conteiner = radioEnergyHelper_conteiner.Install(netDevices_conteiner, sources_conteiner);
  ...
  Simulator::Schedule(Hours(0.00), &GetEnergyRemaining, sources_battery, battery_energy_result_file, 1); // hour
  Simulator::Schedule(Hours(0.00), &GetEnergyRemaining, sources_conteiner,conteiner_energy_result_file, 1); // hour
...
int main (int argc, char *argv[])
{
      ...
      battery_energy_result_file = "battery_energy_results.txt"; 
      conteiner_energy_result_file = "conteiner_energy_results.txt"; 
