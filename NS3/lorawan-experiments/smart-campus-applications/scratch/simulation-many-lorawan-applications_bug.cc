...
// Simulation Code
LoraPacketTracker& runSimulation(int numDevices, int numRandomDevices, NodeContainer endDevicess,  NodeContainer gatewayss, LoraHelper helperr){

  NodeContainer endDevices = endDevicess;
  NodeContainer gateways = gatewayss;
  LoraHelper helper = helperr;
...
int main (int argc, char *argv[])
{
      ...
      for(nSimulation = 0; nSimulation < nSimulationRepeat; nSimulation++){
        ...
        cout << "endDevices: "<< endDevices.Contains(0) << "\n";
        LoraPacketTracker& tracker = runSimulation(nDevices, (int)nDevices_without_dataset/2, endDevices,  gateways, helper); // run simulation
